...
void loop() {
    ...
    int n = snprintf(payload, sizeof(payload),
        "Temperature:%.2f°C Pressure:%ldPa Altitude:%.2fm Vertical Velocity:%.2fm pressureRate:%.2fPa ρ:%.4fkg/m³ q:%.2fPa Mach:%.3f StateID:%u CFlag:%u TimeElapsed:%lusec CNCT:%u",
        pt.temperature,  
        pt.pressure,    
        pt.altitude,
        pt.verticalSpeed, 
    ...
    switch (currentState) {
        ...
        case SystemState::FLIGHT:
            if (!contingencyMode && (altFilterd >= ALT_DEPLOY_THRESHOLD) && (verticalSpeed < 2.0)) {
                ...
                // MAIN DEPLOYMENT: fire once, 250m below apogee
